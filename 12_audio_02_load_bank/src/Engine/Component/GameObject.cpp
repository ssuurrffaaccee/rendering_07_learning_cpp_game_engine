...
GameObject::GameObject(std::string name)
    : Tree::Node(), layer_(0x01), active_(true) {
  set_name(name);
...
Component* GameObject::AddComponent(std::string component_type_name) {
  ...
  if (component_type_instance_map_.find(component_type_name) ==
      component_type_instance_map_.end()) {
...
void GameObject::Foreach(std::function<void(GameObject* game_object)> func) {
  game_object_tree_.Post(
      game_object_tree_.root_node(), [&func](Tree::Node* node) {
        auto n = node;
...
GameObject* GameObject::Find(std::string name) {
  ...
  game_object_tree_.Find(
      game_object_tree_.root_node(),
      [&name](Tree::Node* node) {

...
class Camera : public Component {
 public:
  ...
  /// \param nearClip 近裁剪面
  /// \param farClip  远裁剪面
  void SetPerspective(float fovDegrees, float aspectRatio, float nearClip,
                      float farClip);
  ...
  /// 设置 depth，触发相机排序
  /// \param depth
  /// 相机深度，多相机按从后往前排序，按depth从小往大遍历，depth越大的越上层。
 ...
 private:
  static std::vector<Camera*>
      all_camera_;  // 所有Camera，每一帧都遍历Camera，设置current_camera_。
  static Camera* current_camera_;  // 当前用于渲染的Camera，就是MeshRenderer在计算MVP的时候，用这个Camera的View
                                   // Projection矩阵计算MVP。

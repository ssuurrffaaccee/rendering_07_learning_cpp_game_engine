...
int main(void) {
  ...
  CreateTexture("../../resources/images/urban.cpt");
  ...
  u_diffuse_texture_location =
      glGetUniformLocation(program, "u_diffuse_texture");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glUseProgram(program);
    {
      ...
      glVertexAttribPointer(vpos_location, 3, GL_FLOAT, false, sizeof(Vertex),
                            (float*)(&kVertexRemoveDumplicateVector[0]));
      // 启用顶点Shader属性(a_color)，指定与顶点颜色数据进行关联
      glEnableVertexAttribArray(vcol_location);
      glVertexAttribPointer(vcol_location, 4, GL_FLOAT, false, sizeof(Vertex),
                            ((float*)(&kVertexRemoveDumplicateVector[0]) + 3));
      // 顶启用顶点Shader属性(a_uv)，指定与点UV数据进行关联
      glEnableVertexAttribArray(a_uv_location);
      glVertexAttribPointer(
          a_uv_location, 2, GL_FLOAT, false, sizeof(Vertex),
          ((float*)(&kVertexRemoveDumplicateVector[0]) + 3 + 4));
      // 上传mvp矩阵
      ...
      glDrawElements(
          ...
          (float*)(&kVertexIndexVector[0]));  // 使用顶点索引进行绘制。
    }

...
void UIButton::Update() {
  ...
  // 按钮的图片 与 按钮坐标同步
  Transform* transform_image_normal = dynamic_cast<Transform*>(
      image_normal_->game_object()->GetComponent("Transform"));
  Transform* transform_image_press = dynamic_cast<Transform*>(
      image_press_->game_object()->GetComponent("Transform"));
  ...
  if (Input::GetMouseButtonDown(MOUSE_BUTTON_LEFT)) {
    ...
    if ((mouse_position.x > transform_position.x &&
         mouse_position.x < transform_position.x + image_normal_width) &&
        (mouse_position.y > transform_position.y &&
         mouse_position.y < transform_position.y + image_normal_height)) {
      if (pressed_) {
        return;
      }
      pressed_ = true;

...
FMOD_RESULT AudioStudio::Init() {
  ...
  result = FMOD_Studio_System_Initialize(system_, 1024, FMOD_STUDIO_INIT_NORMAL,
...
FMOD_RESULT AudioStudio::LoadBankFile(string file_name) {
  ...
  return FMOD_Studio_System_LoadBankFile(system_, bank_path.c_str(),
...
AudioStudioEvent* AudioStudio::CreateEventInstance(const char* event_path) {
  ...
  FMOD_RESULT result =
      FMOD_Studio_System_GetEvent(system_, event_path, &event_description);
  if (result != FMOD_OK) {
    DEBUG_LOG_ERROR("FMOD_Studio_System_GetEvent result:{},event_path:{}",
                    int(result), event_path);
    return nullptr;
  }
  ...
  if (result != FMOD_OK) {
    DEBUG_LOG_ERROR(
        "FMOD_Studio_EventDescription_CreateInstance result:{},event_path:{}",
        int(result), event_path);
    return nullptr;
  }

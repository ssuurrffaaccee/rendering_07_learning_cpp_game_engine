        ...
        // 4. 游戏渲染画面
        {
            ...
            if(ReadPixelsQueue::Size()>0)
            {
                ...
                if(texture_id==0){
                    ...
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, read_pixels_buffer->buffer_);__CHECK_GL_ERROR__
                }else{
                    glBindTexture(GL_TEXTURE_2D, texture_id);__CHECK_GL_ERROR__
                    // glPixelStorei(GL_UNPACK_ALIGNMENT, 1);__CHECK_GL_ERROR__
                    glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);__CHECK_GL_ERROR__
                    glTexSubImage2D(GL_TEXTURE_2D,0,0,0,width,height,GL_RGBA,GL_UNSIGNED_BYTE,read_pixels_buffer->buffer_);__CHECK_GL_ERROR__
                }
            ...
            //     std::string file_path = Application::data_path()+"images/btn_power.png";
            //     bool ret = LoadTextureFromFile(file_path.c_str(), &my_image_texture, &my_image_width, &my_image_height);
            //     IM_ASSERT(ret);

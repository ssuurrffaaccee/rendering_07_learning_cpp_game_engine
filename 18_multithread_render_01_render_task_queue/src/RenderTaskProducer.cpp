...
#include "RenderTaskProducer.hpp"
#include "RenderTaskQueue.hpp"
#include "RenderTaskType.hpp"
#include <memory>
...
void RenderTaskProducer::ProduceRenderTaskCompileShader(const char* vertex_shader_source,const char* fragment_shader_source,GLuint& result_shader_program_id){
    auto render_task_compile_shader=std::make_unique<RenderTaskCompileShader>();
    ...
    RenderTaskQueue::Push(render_task_compile_shader.get());
...
void RenderTaskProducer::ProduceRenderTaskCreateVAO(GLuint shader_program_id, const void *positions, GLsizei positions_stride,
    ...
    auto render_task_create_vao=std::make_unique<RenderTaskCreateVAO>();
    ...
    RenderTaskQueue::Push(render_task_create_vao.get());
    //等待任务结束并设置回传结果
...
void RenderTaskProducer::ProduceRenderTaskDrawArray(GLuint shader_program_id, GLuint vao){
    ...
    // consumer will delete render_task_draw_array
}

void RenderTaskProducer::ProduceRenderTaskEndFrame() {
    auto render_task_frame_end=std::make_unique<RenderTaskEndFrame>();
    RenderTaskQueue::Push(render_task_frame_end.get());

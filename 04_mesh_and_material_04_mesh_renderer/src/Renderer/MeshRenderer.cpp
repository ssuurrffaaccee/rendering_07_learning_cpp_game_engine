...
void MeshRenderer::SetMaterial(Material *material) { material_ = material; }

void MeshRenderer::SetMeshFilter(MeshFilter *mesh_filter) {
  mesh_filter_ = mesh_filter;
}

void MeshRenderer::SetMVP(glm::mat4 mvp) { mvp_ = mvp; }
...
void MeshRenderer::Render() {
  ...
  if (vertex_array_object_ == 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 mesh_filter_->mesh()->vertex_num_ * sizeof(MeshFilter::Vertex),
                 mesh_filter_->mesh()->vertex_data_, GL_STATIC_DRAW);
    ...
    glBufferData(
        ...
        mesh_filter_->mesh()->vertex_index_num_ * sizeof(unsigned short),
        mesh_filter_->mesh()->vertex_index_data_, GL_STATIC_DRAW);
  ...
  glUseProgram(gl_program_id);
  {
    ...
    // 上传mvp矩阵
    glUniformMatrix4fv(glGetUniformLocation(gl_program_id, "u_mvp"), 1,
                       GL_FALSE, &mvp_[0][0]);

    // 从Pass节点拿到保存的Texture
    std::vector<std::pair<std::string, Texture2D *>> textures =
        material_->textures();
    for (int texture_index = 0; texture_index < textures.size();
         ++texture_index) {
      GLint u_texture_location = glGetUniformLocation(
          gl_program_id, textures[texture_index].first.c_str());
      // 激活纹理单元0
      glActiveTexture(GL_TEXTURE0 + texture_index);
      // 将加载的图片纹理句柄，绑定到纹理单元0的Texture2D上。
      glBindTexture(GL_TEXTURE_2D,
                    textures[texture_index].second->gl_texture_id());
    ...
    glBindVertexArray(vertex_array_object_);
    {
      glDrawElements(GL_TRIANGLES, mesh_filter_->mesh()->vertex_index_num_,
                     GL_UNSIGNED_SHORT,

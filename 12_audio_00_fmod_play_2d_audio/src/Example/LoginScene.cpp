...
void LoginScene::CreateSounds() {
  FMOD_RESULT result;
  // 战斗背景音乐
  result = Audio::CreateSound(
      (Application::data_path() + "audio/fmod_play_wav/war_bgm.wav").c_str(),
      FMOD_2D | FMOD_LOOP_NORMAL, nullptr, &sound_1);
  DEBUG_LOG_INFO("CreateSound ret code {}", int(result));
  // 刀攻击音效
  result = Audio::CreateSound(
      (Application::data_path() + "audio/fmod_play_wav/knife_attack.wav")
          .c_str(),
      FMOD_2D, nullptr, &sound_2);
  DEBUG_LOG_INFO("CreateSound ret code {}", int(result));
  // 魔法攻击音效
  result = Audio::CreateSound(
      (Application::data_path() + "audio/fmod_play_wav/magic_attack.wav")
          .c_str(),
      FMOD_2D, nullptr, &sound_3);
  DEBUG_LOG_INFO("CreateSound ret code {}", int(result));
}
...
void LoginScene::PlayPauseSound(FMOD_SOUND* sound, FMOD_CHANNEL** channel) {
  ...
  // 判断音效是否暂停
  result = FMOD_Channel_GetPaused(
      *channel,
      &paused);  // 音效播放完毕后，channel被回收，返回 FMOD_ERR_INVALID_HANDLE
  DEBUG_LOG_INFO("FMOD_Channel_GetPaused ret code {}", int(result));
  switch (result) {
    case FMOD_OK:
      // 暂停播放

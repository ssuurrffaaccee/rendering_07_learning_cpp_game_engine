...
void LoginScene::CreateFishSoupPot() {
  ...
  // 挂上 Transform 组件
  transform_fishsoup_pot_ =
      dynamic_cast<Transform*>(go->AddComponent("Transform"));
...
void LoginScene::CreateFont() {
  ...
  for (auto character : character_vec) {
    ...
    vector<unsigned short> index_vector = {0, 1, 2, 0, 2, 3};
    // 创建模型 GameObject
    ...
    // 挂上 MeshFilter 组件
    auto mesh_filter =
        dynamic_cast<MeshFilter*>(go->AddComponent("MeshFilter"));
...
void LoginScene::CreateUI() {
  ...
  // 挂上 Transform 组件
  auto transform_camera_ui =
      dynamic_cast<Transform*>(go_camera_ui->AddComponent("Transform"));
  transform_camera_ui->set_position(glm::vec3(0, 0, 10));
  // 挂上 Camera 组件
  auto camera_ui =
      dynamic_cast<Camera*>(go_camera_ui->AddComponent("UICamera"));
  ...
  camera_ui->SetOrthographic(-Screen::width() / 2, Screen::width() / 2,
                             -Screen::height() / 2, Screen::height() / 2, -100,
                             100);
  ...
  // 挂上 UIImage 组件
  auto ui_image_mod_bag =
      dynamic_cast<UIImage*>(go_ui_image->AddComponent("UIImage"));
  ...
  // 挂上 Transform 组件
  auto transform_ui_mask =
      dynamic_cast<Transform*>(go_ui_mask->AddComponent("Transform"));
  // 挂上 UIMask 组件
  auto ui_mask_mod_bag =
      dynamic_cast<UIMask*>(go_ui_mask->AddComponent("UIMask"));
  ui_mask_mod_bag->set_texture(
      Texture2D::LoadFromFile("images/mod_bag_mask.cpt"));
  ...
  // 挂上 Transform 组件
  auto transform_ui_text =
      dynamic_cast<Transform*>(go_ui_text->AddComponent("Transform"));
  ...
  auto ui_image_button_image_normal =
      dynamic_cast<UIImage*>(go_button_image_normal->AddComponent("UIImage"));
  ui_image_button_image_normal->set_texture(
      Texture2D::LoadFromFile("images/btn_power.cpt"));
  // 创建按钮按下状态图片
  ...
  auto ui_image_button_image_normal_press = dynamic_cast<UIImage*>(
      go_button_image_normal_press->AddComponent("UIImage"));
  ui_image_button_image_normal_press->set_texture(
      Texture2D::LoadFromFile("images/btn_power_press.cpt"));
  // 创建按钮
  auto go_ui_button = new GameObject("button");
  go_ui_button->set_layer(0x02);
  auto transform_ui_button =
      dynamic_cast<Transform*>(go_ui_button->AddComponent("Transform"));
  transform_ui_button->set_position({100.f, -200.f, 0});
  auto ui_button =
      dynamic_cast<UIButton*>(go_ui_button->AddComponent("UIButton"));
...
void LoginScene::Update() {
  ...
  // 旋转相机
  if (Input::GetKeyDown(KEY_CODE_LEFT_ALT) &&
    ...
    glm::mat4 rotate_mat4 = glm::rotate(
        old_mat4, glm::radians(degrees),
        glm::vec3(
            0.0f, 1.0f,
            0.0f));  // 以相机所在坐标系位置，计算用于旋转的矩阵，这里是零点，所以直接用方阵。
    glm::vec4 old_pos = glm::vec4(transform_camera_1_->position(), 1.0f);
    glm::vec4 new_pos =
        rotate_mat4 * old_pos;  // 旋转矩阵 * 原来的坐标 = 相机以零点做旋转。
  ...
  // 鼠标滚轮控制相机远近
  transform_camera_1_->set_position(transform_camera_1_->position() *
                                    float(10 - Input::mouse_scroll()) / 10.f);
}
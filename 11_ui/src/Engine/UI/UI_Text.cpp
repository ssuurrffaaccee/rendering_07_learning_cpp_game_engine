...
RTTR_REGISTRATION {
  registration::class_<UIText>("UIText").constructor<>()(
  ...
  if (dirty_) {
    ...
    for (int i = 0; i < character_vec.size(); ++i) {
      ...
      unsigned short width =
          (character->right_bottom_x_ - character->left_top_x_) *
          font_->font_texture()->width();
      unsigned short height =
          (character->right_bottom_y_ - character->left_top_y_) *
          font_->font_texture()->height();
      // 因为FreeType生成的bitmap是上下颠倒的，所以这里UV坐标也要做对应翻转，将左上角作为零点。
      vertex_vector.insert(
          vertex_vector.end(),
          {{{x, 0.0f, 0.0f},
            color_,
...
void UIText::OnPreRender() { Component::OnPreRender(); }

void UIText::OnPostRender() { Component::OnPostRender(); }

...
Font* Font::LoadFromFile(std::string font_file_path, unsigned short font_size) {
  ...
  // 读取 ttf 字体文件
  ifstream input_file_stream(Application::data_path() + font_file_path,
                             ios::in | ios::binary);
  ...
  FT_Error error = FT_New_Memory_Face(
      ft_library, (const FT_Byte*)font_file_buffer, len, 0, &ft_face);
  if (error != 0) {
  ...
  unsigned char* pixels = (unsigned char*)malloc(font->font_texture_size_ *
                                                 font->font_texture_size_);
  memset(pixels, 0, font->font_texture_size_ * font->font_texture_size_);
  font->font_texture_ =
      Texture2D::Create(font->font_texture_size_, font->font_texture_size_,
                        GL_RED, GL_RED, GL_UNSIGNED_BYTE, pixels);
  delete pixels;
...
void Font::LoadCharacter(char ch) {
  ...
  // 计算新生成的字符，在图集中的排列。
  if (font_texture_fill_x + ft_bitmap.width >=
      font_texture_size_) {  // 从左上角往右上角填充，满了就换一行。
  ...
  font_texture_->UpdateSubImage(font_texture_fill_x, font_texture_fill_y,
                                ft_bitmap.width, ft_bitmap.rows, GL_RED,
                                GL_UNSIGNED_BYTE, ft_bitmap.buffer);

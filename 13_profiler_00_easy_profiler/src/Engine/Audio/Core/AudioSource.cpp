#include "Engine/Audio/Core/AudioSource.hpp"
...
void AudioSource::Set3DMode(bool mode_3d) {
  ...
  if (result != FMOD_OK) {
    spdlog::error("AudioSource::Set3DMode FMOD_Channel_SetMode result:{}",
                  int(result));
  }
}
...
void AudioSource::Play() {
  ...
  switch (result) {
    ...
    case FMOD_ERR_CHANNEL_STOLEN:  // 音效播放完毕后，channel被回收且被分配给其他Sound。
      // 播放音效
      result = AudioCore::PlaySound(audio_clip_->fmod_sound(), nullptr, false,
                                &fmod_channel_);
      break;
    default:
      break;
...
void AudioSource::Pause() {
  ...
  spdlog::error("AudioSource::Paused FMOD_Channel_GetPaused result:{}", int(result));
}
...
void AudioSource::Stop() {
  ...
  spdlog::error("AudioSource::Stop FMOD_Channel_Stop result:{}", int(result));
}
...
void AudioSource::SetLoop(bool mode_loop) {
  ...
  FMOD_RESULT result = FMOD_Channel_SetMode(fmod_channel_, fmod_mode_);
  if (result != FMOD_OK) {
    spdlog::error("AudioSource::SetLoop FMOD_Channel_SetMode result:{}",
                  int(result));
  }
}
